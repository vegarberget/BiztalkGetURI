## - Name GetreceiveportURI.ps1
## - Eksample: ./getreceiveporturi.ps1 -port FILE 
Param(
...
$MyNewObject = foreach($item in ($BTSexp.receivePorts))
{
 #Building your PowerShell PSObject item:
    $newPSitem = New-Object PSObject -Property @{
  seq = $y.ToString("000");
  Application = $item.Application.name;
  PortType = $item.primaryreceivelocation.TransportType.Name;
  URI = $item.primaryreceivelocation.Address;
  Port = $item.Name;
 };
